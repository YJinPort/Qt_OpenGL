...
void Widget::paintGL() {
    ...
    if(bProjection != 0) glTranslatef(0, 0, -2);

    glRotatef(xAngle, 1.0f, 0.0f, 0.0f);
    glRotatef(yAngle, 0.0f, 1.0f, 0.0f);
    glRotatef(zAngle, 0.0f, 0.0f, 1.0f);

    if(bObject == 0) drawPyramid();
    else {
        GLUquadricObj* pQuad;
        pQuad = gluNewQuadric();
...
void Widget::keyPressEvent(QKeyEvent* event) {
    ...
    case Qt::Key_B:
        ...
        if(bProjection == 0) bNear = -1, bFar = 1;
        else bNear = 1, bFar = 10;
        break;
    }

    QString str;
    str = QString("(%.0f, %.0f, %.0fk, %.1f, %.1f, %.1f, %.1f, %.1f, %.1f)").arg(xAngle).arg(yAngle).arg(zAngle).arg(left).arg(right).arg(bottom).arg(top)
            .arg(bNear).arg(bFar);
